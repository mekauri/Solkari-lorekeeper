        ...
        try {
            ...
            if (!$isClaim) {
                foreach ($prompt->rewards as $reward) {
                    addAsset($promptRewards, ($reward->rewardable_type == 'Exp' || $reward->rewardable_type == 'Points') ? $reward->rewardable_type : $reward->reward, $reward->quantity);
                }
            }
            ...
            foreach ($characters as $key => $c) {
                ...
                if (isset($data['character_is_focus']) && $data['character_is_focus'][$c->id] && $submission->prompt_id) {
                    if ($prompt->level_req) {
                        if (!$c->level || $c->level->current_level < $prompt->level_req) {

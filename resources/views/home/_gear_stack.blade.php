                ...
                <li class="list-group-item">
                    ...
                    @if ($stack->character_id != null && $diff < Carbon\Carbon::now())
                        <a class="card-title h5 collapse-title" data-toggle="collapse" href="#attachForm">
                            @if ($stack->user_id != $user->id)
                                [ADMIN]
                            @endif Detach Gear from Character
                        </a>
                    ...
                    @elseif($stack->character_id == null || $diff < Carbon\Carbon::now())
                        <a class="card-title h5 collapse-title" data-toggle="collapse" href="#attachForm">
                            @if ($stack->user_id != $user->id)
                                [ADMIN]
                            @endif Attach Gear to Character
                        </a>
                ...
                @if ($stack->gear->parent_id && $stack->gear->cost && $stack->gear->currency_id <= 0)
                    <li class="list-group-item">
                        ...
                        <p>Upgrade costs {{ $stack->gear->cost }}
                            @if ($stack->gear->currency_id != 0)
                                <img src="{!! $stack->gear->currency->iconurl !!}"> {!! $stack->gear->currency->displayName !!}.
                            @else
                                stat points.
                            @endif

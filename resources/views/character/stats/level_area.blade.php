...
@section('profile-content')
    ...
    <div class="container text-center">
        ...
        @if ($next)
            ...
            <div class="progress">
                <div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="{{ $character->level->current_exp }}" aria-valuemin="0" aria-valuemax="{{ $next->exp_required }}" style="width:{{ $width }}%">
                    {{ $character->level->current_exp }}/{{ $next->exp_required }}
                </div>
            </div>
            @if ($character->level->current_exp >= $next->exp_required && Auth::check() && Auth::user()->id == $character->user_id)
                <div class="text-center m-1">
                    <b>
                        <p>You have enough EXP to advance to the nex level!</p>
                    </b>
                </div>
